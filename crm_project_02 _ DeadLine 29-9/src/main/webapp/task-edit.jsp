			...
			<div class="container-fluid">
				<div class="row bg-title">
					<div class="col-lg-3 col-md-4 col-sm-4 col-xs-12">
						<h4 class="page-title">Chỉnh sữa công việc</h4>
					</div>
				</div>
				<!-- /.row -->
				<!-- .row -->
						...
						<div class="white-box">
							<form class="form-horizontal form-material"
								...
								<div class="form-group">
									...
									<div class="col-md-12">
										<input type="text" readonly value="${job.project.name}"
											class="form-control form-control-line"> <input
											type="hidden" value="${job.id}" name="id_job">
									</div>
								</div>
								<div class="form-group">
									<label class="col-md-12">Tên công việc</label>
									<div class="col-md-12">
										<input type="text" readonly value="${job.jobName}"
										...
										<select class="form-control form-control-line"
											...
											<c:forEach var="items" items="${listStatus}">
												<option value="${items.id}"
													<c:if test="${items.id eq job.status.id}">selected="selected"</c:if>">${items.name}</option>
											</c:forEach>
										</select>
									</div>
								</div>
